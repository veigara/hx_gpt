...
def save_agent(user_name,json_str) ->None:
    """保存智能体文件
    {"id": "1b58a6b59a4a4bcba07078d415d74f0b", "title": "文言文翻译", "content": [{"role": "assistant", "chatText": "用文言文表达输入的内容"}, {"role": "user", "chatText": "示例：我很开心，因为今天出去玩了"}], "model_name": "llama-3.2-90b-vision", "temperature": 0.8, "top_p": 0.3, "max_tokens": 84492, "presence_penalty": 1.5, "frequency_penalty": 1.7}
    """
    try:
...
def load_agent(file_user_name,id) ->str:
    """从文件目录中获取智能体文件的内容
    """
    try:
...
def get_file_path(file_user_name,id):
    """获取智能体的文件路径
    """
    return  os.path.join(DEFALUE_AGENT_DIR,f'{id}.json') if file_user_name==SYS_USER_NAME else os.path.join(AGENT_DIR,file_user_name,f'{id}.json') 
...
def get_sys_agent():
    ...
    for file in os.listdir(os.path.join(DEFALUE_AGENT_DIR)):
                if file.endswith(".json"):
                    with open(os.path.join(DEFALUE_AGENT_DIR,file), "r",encoding='utf-8')as f:
                        agent_data = json.load(f)
                        agent_data['content'] = []
                        # 系统内置，不允许编辑
